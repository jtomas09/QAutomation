 ...
 *
 * Centralizes all devicectl JSON parsing for the Runner JVM. Replaces indexOf/substring/regex
 * extraction throughout CoreDeviceTunnelManager, IosPreflightManager, and IOSDeviceScanner.
 *
 * Device lookup is always keyed on {@code hardwareProperties.udid} (physical UDID), which is
 * the canonical identifier used by Appium. This avoids the Xcode 26 regression where the same
 * UDID appears earlier in {@code connectionProperties.potentialHostnames} and causes window-based
 * extraction to miss the {@code identifier} field that follows {@code hardwareProperties}.
 *
...
public final class DevicectlParser {
    ...
    public enum TransportType {
        /** USB cable — daemon restarts (killall remotedeviced) are safe. */
        WIRED,
    ...
    public static final class DeviceInfo {
        /** {@code identifier} — CoreDevice UUID (RFC 4122, 36 chars). Used for devicectl operations. */
        public final String        coreDeviceId;
        /** {@code hardwareProperties.udid} — physical UDID sent to Appium as {@code appium:udid}. */
        ...
        /** iOS version string, e.g. {@code "26.5"}. From {@code deviceProperties.osVersionNumber}. */
     ...
     *
     * Returns {@code null} when the device is not found, the JSON is invalid, or input is blank.
     * Never searches the JSON text as a string — the lookup is always structural via Jackson.
     */
     ...
     * Used by IOSDeviceScanner to resolve CoreDevice UUIDs to physical UDIDs.
     */
    ...
    private static DeviceInfo fromNode(JsonNode device) {
        ...
        // pairingState: check connectionProperties first, then deviceProperties,
        // then Xcode 26 localHostEnrollmentState alias
        ...
        // OS version: deviceProperties.osVersionNumber is the most complete source;
        // fall back to hardwareProperties fields for older Xcode versions
        ...
        // developerModeEnabled: explicit boolean field, or status string "enabled"
        boolean devMode;
        ...
        // screenLocked: multiple field names across Xcode versions
